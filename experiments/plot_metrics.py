...
# --- Configuration ---
FILE_BASELINE = 'results/output_base.txt'
FILE_PH = 'results/output_ph.txt'
...
def main():
    ...
    print("\n" + "="*110)
    print("METRICS (%) - By Noise Level")
    print("="*110)
    print(f"{'Noise':<8} {'Precision (%)':<24} {'Recall (%)':<24} {'F1 Score (%)':<24} {'Accuracy (%)':<24}")
    print(f"{'Level':<8} {'Base':<12} {'Ours':<12} {'Base':<12} {'Ours':<12} {'Base':<12} {'Ours':<12} {'Base':<12} {'Ours':<12}")
    print("-"*110)

    for i, noise in enumerate(x_sorted):
        print(f"{noise:<8} {prec_base[i]*100:<12.2f} {prec_ph[i]*100:<12.2f} {rec_base[i]*100:<12.2f} {rec_ph[i]*100:<12.2f} {f1_base[i]*100:<12.2f} {f1_ph[i]*100:<12.2f} {acc_base[i]*100:<12.2f} {acc_ph[i]*100:<12.2f}")

    print("-"*110)
    print(f"{'Mean':<8} {np.mean(prec_base)*100:<12.2f} {np.mean(prec_ph)*100:<12.2f} {np.mean(rec_base)*100:<12.2f} {np.mean(rec_ph)*100:<12.2f} {np.mean(f1_base)*100:<12.2f} {np.mean(f1_ph)*100:<12.2f} {np.mean(acc_base)*100:<12.2f} {np.mean(acc_ph)*100:<12.2f}")
    print("="*110 + "\n")

    fig, axs = plt.subplots(2, 2, figsize=(12, 10))

    def style_plot(ax, y_base, y_ph, title, show_xlabel=True, show_ylabel=True):
    ...
    style_plot(axs[0, 0], prec_base, prec_ph, 'Precision', show_xlabel=False, show_ylabel=True)
    style_plot(axs[0, 1], rec_base, rec_ph, 'Recall', show_xlabel=False, show_ylabel=False)
    style_plot(axs[1, 0], f1_base, f1_ph, 'F1 Score', show_xlabel=True, show_ylabel=True)
    style_plot(axs[1, 1], acc_base, acc_ph, 'Accuracy', show_xlabel=True, show_ylabel=False)
    ...
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.1, hspace=0.25, wspace=0.15)
