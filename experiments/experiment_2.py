...
from common.colors import pth_color_str, baseline_color_str, baseline_color
...
if __name__ == '__main__':
    ...
    for n_pt_idx, args.n_point_line_2 in enumerate(range(200,190,-50)):
        ...
        for sim_run in range(0, args.num_sim_rounds):
            logging.info(f"Pointcount Line 2: {args.n_point_line_2}, run:{sim_run}")
            ...
            coordinates = generate_line(slope=args.line_1_slope, intercept=args.line_1_intercept + offset, noise_lvl=noise_lvl, num_points= args.n_point_line_1)
            coordinates += generate_line(slope=args.line_1_slope, intercept=args.line_2_intercept - offset, noise_lvl = noise_lvl, num_points= args.n_point_line_2)
            image = generate_image(coordinates)
            edges = np.array(image)
            original_image = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)

            hough_transformer = TopologicalHoughTransform(image, value_threshold=150, pers_limit=105, three_periods=True)
            img_with_lines_PH = draw_lines_on_image(hough_transformer)

            img_with_lines, lines = baseline_detect_lines(original_image=original_image, edges=edges, threshold=75)

            for line in lines:
                pt1, pt2 = line_to_pts(line[0])
                draw_dashed_line(img_with_lines_PH, pt1, pt2, color=baseline_color)

            fig, axs = plt.subplots(1, 3, figsize=(12, 4))
            axs[0].imshow(img_with_lines_PH)
            axs[0].title.set_text('Image Space')
            handles = [plt.Line2D([0], [0], color=baseline_color_str, label='baseline'),
                       plt.Line2D([0], [0], color=pth_color_str, label='our method')]

            plot_persistence_diagram(hough_transformer,
                                     ax=axs[1], show_limit=True,
                                     show_nums=False)

            my_true_lines, my_other_lines=hough_transformer.plot_hough_with_loci(true_lines=true_lines, other_lines=lines, show='none', my_ax=axs[2])
            cm = get_conf_matrix(args.n_point_line_2, my_true_lines, my_other_lines)
            cm_baseline = [[cm_baseline[i][j] + cm[i][j] for j in range(len(cm_baseline[i]))] for i in range(len(cm_baseline))]

            cm = get_conf_matrix(args.n_point_line_2, my_true_lines, hough_transformer.get_lines())
            cm_PH = [[cm_PH[i][j] + cm[i][j] for j in range(len(cm_PH[i]))] for i in range(len(cm_PH))]

            plt.tight_layout()
            plt.savefig(os.path.join(args.output_directory, 'tmp', f'plot_{args.n_point_line_2}{sim_run}.png'))

            for line in lines:
                logging.debug(f"Lines found with opencv: {line}")

        logging.debug(f"Confusion Matrix Baseline for d={args.n_point_line_2}: {cm_baseline}")
        logging.debug(f"Confusion Matrix PH for d={args.n_point_line_2}: {cm_PH}")
        cms_PH.append(cm_PH)
