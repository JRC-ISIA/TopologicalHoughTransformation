...
if __name__ == '__main__':
    ...
    for noise in range(8, 10):
        for j in range(0, 10):
            coordinates = generate_line(slope=slope, intercept=intercept1, noise_lvl=noise, num_points= 150)
            coordinates += generate_line(slope=slope, intercept=intercept2,noise_lvl = noise,num_points= 120)
            image = generate_image(coordinates)
            edges = np.array(image)
            original_image = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)

            fig, axs = plt.subplots(3, 2, figsize=(10, 12), gridspec_kw={'height_ratios': [5,2,5]})
            fig.suptitle('Noise Level: ' + str(noise))
            ...
            img_with_lines, lines = baseline_detect_lines(original_image=original_image, edges=edges, threshold=20)
    ...
    images = [Image.open(workingfolder_path+filename) for filename in filenames]
    images[0].save(subfolder_path+'plots.gif',
                   save_all=True, append_images=images[1:], optimize=False, duration=700, loop=0)

    fig, axs = plt.subplots(1, 2, figsize=(10, 5))
    fig.suptitle('Line search results: ')

    sns.heatmap(cm_PH, annot=True, fmt='d', cmap='magma', ax=axs[0], annot_kws={"size": 10}, cbar=False,
                square=True, linewidths=0.5, linecolor='gray', xticklabels=['found', 'not found'],
                yticklabels=['existing', 'not existing'])

    axs[0].set_xlabel(f'Predicted')
    axs[0].set_ylabel('Actual')
    axs[0].set_title('Our method')

    sns.heatmap(cm_baseline, annot=True, fmt='d', cmap='magma', ax=axs[1], annot_kws={"size": 10}, cbar=False,
                square=True, linewidths=0.5, linecolor='gray', xticklabels=['found', 'not found'],
                yticklabels=['existing', 'not existing'])

    axs[1].set_xlabel(f'Predicted')
    axs[1].set_ylabel('Actual')
    axs[1].set_title('Baseline method')

    plt.tight_layout()
    plt.savefig(workingfolder_path + 'Confusion Matrix')
    plt.show()
    plt.close()
